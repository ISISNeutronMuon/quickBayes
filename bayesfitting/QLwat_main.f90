...
C-----------------------------------------------------------------------
C
      SUBROUTINE QLwat(numb,x_in,y_in,e_in,reals,opft,
...
cf2py intent(out) :: yprob                               !probability values
     ...
     1 PRMSV(7,4,m_sp),SIGSV(7,4,m_sp)
      REAL  AJ0Q(m_sp),AJ1Q(m_sp),AJ2Q(m_sp)
      INTEGER INDX(m_p)
      DATA AROT /0.98/
      prog='w'
      CALL init_paras
...
c     reals = [efix, theta[isp], rscl, bnorm]
      ...
      fileout1=''
      l_lpt=l_fn+8
      lptfile(1:l_lpt)=sfile(1:l_fn)//'_QLw.lpt'
      l_file=l_fn+8
      fileout1(1:l_lpt)=sfile(1:l_fn)//'_QLw.ql1'
      l_title=9
      title='<unknown>'
      l_user=9
      user='<unknown>'
...
1120   format(' qlm> width 1 fixed at ',f10.5)
      ...
      CALL FileInit(1,ISP)
      CALL BESINT(AJ0Q(ISP),AJ1Q(ISP),AJ2Q(ISP),AROT*QAVRG(ISP))
      AJ0=AJ0Q(ISP)
      AJ1=AJ1Q(ISP)
      AJ2=AJ2Q(ISP)
      CALL REFINAw(GRAD,HESS,DPAR,3+NFEW,DETLOG,INDX,COVAR)
      goto 2
   1  CALL SEARCHw(GRAD,HESS,DPAR,NFEW,INDX,COVAR,FITP)
   2  NPARMS=4+3*NFEW
      CHIOLD=CCHI(FITP)
      ...
      CDIFMN=0.003
      do I=1,200
       CALL REFINEw(GRAD,HESS,NPARMS,DETLOG,INDX,COVAR,STEPSZ)
   ...
   3  continue
      CALL REFINEw(GRAD,HESS,NPARMS,DETLOG,INDX,COVAR,0.7)
      CALL ERRBARw(COVAR,NP,SIGPAR,COV12)
      call message('before SEEFITw ')
      CALL SEEFITw(SIGPAR,CNORM,PRMSV(1,NFEW+1,ISP),SIGSV(1,NFEW+1,ISP))
      CALL OUTPRM(FITP,COVAR,NPARMS,NFEW,CNORM)
      CALL REFINEw(GRAD,HESS,NPARMS,DETLOG,INDX,COVAR,0.25)
      call message('before PROBNw  ')
      CALL PROBNw(CNORM,NDAT,DETLOG,NFEW,PROBLG)
      noff=NDAT*NFEW
      do n=1,NDAT
       yfit(noff+n)=FIT(n)
      end do 
      ...
      call message('before PRBOUT  ')
      CALL PRBOUT(PRBSV,1,ISP,POUT)
      do n=1,2
       yprob(n)=POUT(n,isp)
      end do
      kill=0

...
def LUDCMP(A, N, NP):
    ...
    for I in get_range(1, N):
        ...
        for J in get_range(1, N):
            if round_sig(abs(A(I, J))) > AAMAX:
                AAMAX = round_sig(abs(A(I, J)))

        if AAMAX == 0.0:
            print(' Singular matrix!')
            return
        VV.set(I, round_sig(1.0 / AAMAX))
        ...
        for I in get_range(J, N):
            ...
            if J > 1:
                for K in get_range(1, J - 1):
                    SUM = round_sig(SUM - round_sig(A(I, K))
                                    * round_sig(A(K, J)))
                A.set(I, J, SUM)

            DUM = round_sig(round_sig(VV(I)) * abs(round_sig(SUM)))
            if round_sig(DUM) >= round_sig(AAMAX):
                IMAX = I
                AAMAX = round_sig(DUM)

        if J != IMAX:
            for K in get_range(1, N):
                DUM = round_sig(A(IMAX, K))
                A.set(IMAX, K, round_sig(A(J, K)))
                A.set(J, K, DUM)

            D = -D
            VV.set(IMAX, round_sig(VV(J)))

...
c      write(*,*)'waaaaa', NFEW
c      CALL VMLTRC(WORK,FR2PIK(1,2),NFT2,FWRK)
c      CALL VMLTRC(TWOPIK,FWRK,NFT2,WORK)
c      CALL VMLTIC(WORK,NFT2,WORK)
c      CALL FOUR2(FWRK,NFFT,1,-1,-1)
c      CALL DEGRID(FWRK,DDDPAR(1,4))
c      CALL FOUR2(WORK,NFFT,1,-1,-1)
c      CALL DEGRID(WORK,FWRK)
c      CALL VRDOTR(RESID,FWRK,NDAT,HESS(4,4))
c      CALL VCOPY(FR2PIK,FWRK,NFFT+2)
c      CALL FOUR2(FWRK,NFFT,1,-1,-1)
c      CALL DEGRID(FWRK,DDDPAR(1,3))
c      CALL VCOPY(FR2PIK(1,2),FWRK,NFFT+2)
c      CALL FOUR2(FWRK,NFFT,1,-1,-1)
c      CALL DEGRID(FWRK,WORK)
c      CALL VRDOTR(RESID,WORK,NDAT,HESS(3,4))
...
c        CALL FOUR2(WORK,NFFT,1,-1,-1)
c        CALL DEGRID(WORK,FWRK)
c        CALL VRDOTR(RESID,FWRK,NDAT,SM)
c        HESS(J+1,J+1)=-AJ*SM
      end do
...
C
 ...
 170    FORMAT(5X,' FWHM        =   ',F13.2,'  +- ',F11.2,' ueV')
        WRITE(53,370) FITP(J),WSCL,SIGPAR(J), ERRSCL
 370    FORMAT(5X,' FWHMdfdsa   =   ', F13.2, '   ', F13.2, '     ',
     1     F11.2, '    ', F11.2)

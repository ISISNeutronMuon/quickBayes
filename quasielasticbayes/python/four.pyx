...
from quasielasticbayes.python.fortran_python import get_range, complex_zeros
#from math import pi
import numpy as np
...
def FOUR2_raw(np.ndarray[np.float_t] DATA,int N,int NDIM,int ISIGN,int IFORM):
      return FOUR2_NEG_IFORM(DATA, N,NDIM,ISIGN,IFORM) # we know all of the calls have IFORM = -1
